...
"#;
    ...
    let q = report.results.get("op_0002").expect("query result");
    assert_eq!(q.kind, "query");
}

#[test]
fn schdoc_ops_source_runtime_rejects_unsupported_op() {
    let input = schdoc_fixture_path(
        "myriadrf_LimeSDR-XTRX__hardware_1v0_Schematics__03_Clock_Diagram.SchDoc",
    );
    let mut doc = SchDoc::open(&input).expect("open schdoc fixture");
    ...
    let src = r#"
add_component { lib_reference: "OPS_E2E_R2", designator: "R902" }
add_parameter { name: "MFG", text: "ACME" }
"#;

    let err = apply_ops_source_schdoc(&mut doc, src).expect_err("should fail at schdoc runtime");
    assert!(err.to_string().contains("operation is not supported for SchDoc"));
}
