//! Reverse generation: produce `*-spec` source from
...
/// Result of dumping an IntLib into separate library specs.
pub struct IntLibDump {
    /// `.schlib-spec` source containing embedded schematic symbols.
    pub schlib_spec: Option<String>,
    /// `.pcblib-spec` source containing embedded PCB footprints.
    pub pcblib_spec: Option<String>,
}

/// Dump an IntLib's embedded libraries as separate SchLib and PcbLib spec files.
pub fn dump_intlib(lib: &IntLib) -> Result<IntLibDump, altium_format::AltiumFormatError> {
    let mut schlib_out = String::new();
    let mut pcblib_out = String::new();

    for schlib in lib.schlibs() {
        for comp in &schlib.components()? {
            dump_component(&mut schlib_out, comp);
            schlib_out.push('\n');
        }
    }
    ...
    for pcblib in lib.pcblibs() {
        for name in pcblib.footprint_names() {
            ...
            dump_footprint(&mut pcblib_out, &fp);
            pcblib_out.push('\n');
        }
    }

    let schlib_spec = if schlib_out.is_empty() {
        None
    } else {
        Some(schlib_out)
    };
    let pcblib_spec = if pcblib_out.is_empty() {
        None
    } else {
        Some(pcblib_out)
    };
