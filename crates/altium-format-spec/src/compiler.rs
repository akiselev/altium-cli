...
use crate::model::{
    ...
    FootprintMapSpec, FootprintRef as ModelFootprintRef, FootprintSpec, GraphicProperties,
    GraphicSpec, GraphicType, HarnessConnectorSpec, JunctionSpec, LayerSpec, LibraryUpdateSpec,
    NetLabelSpec, NetSpec, NoConnectSpec, NoteSpec, OutputGroupSpec, OutputSpec, PadSpec,
    ParamVariationSpec, ParameterSetSpec, ParameterSpec, PartSpec, PcbDocClassSpec,
    ...
    PlacementRuleSpec, PlacementSpec, PortSpec, PowerObjectSpec, PowerSpec, PrjPcbSpec, ProbeSpec,
    ProjectSpec, RoutingSpec, SchDocComponentSpec, SchDocObjectSpec, SchDocSpec, SchLibSpec,
    SheetEntrySpec, SheetSpec, SheetSymbolSpec, SignalHarnessSpec, SpecDomain, SpecModel,
    SymbolRef, UnplacedStrategy, VariantSpec, VariationSpec, WireSpec,
};
...
A, B, C, D, E, Letter, Legal, Tabloid, OrcadA, OrcadB, OrcadC, OrcadD, or OrcadE)"
    ...
    fn compile_pcbdoc_net(
        ...
        let routing_style = get_string_opt(&props, "routing_style");
    ...
    fn compile_pcbdoc_rule(&mut self, decl: &RuleDecl) -> Result<PcbDocRuleSpec, SpecError> {
        ...
        // NOTE: "gap" is intentionally included in properties — PCB rule parameters
        // (Clearance.gap, BoardOutlineClearance.gap, etc.) are read from `properties`
        // by the spec_compiler. Excluding "gap" here would cause those fields to
        // silently read as 0.0.
        let well_known = ["kind", "enabled", "priority", "scope", "properties"];
        let mut properties = indexmap::IndexMap::new();
        for (k, v) in &raw {
