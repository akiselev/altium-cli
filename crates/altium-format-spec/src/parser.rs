...
use super::ast::{
    AliasDecl, AnnotationBlockDecl, AnnotationKey, BlockAnnotation, BoardDecl, BoardItem, CallArg,
    ...
    PlacementSeparateDecl, PolygonDecl, PowerDecl, ProjectDecl, ProjectItem, Property, RoutingDecl,
...
impl<'a> SpecParser<'a> {
    ...
    fn try_eat_property_key(&mut self) -> Option<Spanned<String>> {
        ...
        let name = match self.current_kind().clone() {
            ...
            TokenKind::Power => "power".to_string(),
            TokenKind::Board => "board".to_string(),
            TokenKind::Sheet => "sheet".to_string(),
            TokenKind::PadNet => "pad_net".to_string(),
...
"#,
    ...
    fn test_call_no_args() {
        ...
        if let SpecItem::LetBinding(b) = &f.items[0].node {
            assert!(
                matches!(&b.value.node, Expr::Call { name, args } if name == "foo" && args.is_empty())
            );
        } else {
            panic!("expected LetBinding");
        }
