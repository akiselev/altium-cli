...
///
/// Strict AD26 parser: requires 50-byte layout.
pub(crate) fn parse_fill(data: &[u8]) -> crate::Result<crate::backing_store::RecordOrigin> {
    use crate::error::AltiumError;
    use crate::backing_store::{BinaryOrigin, FieldSpan};

    if data.len() < 50 {
        return Err(AltiumError::Parse(format!(
            "fill data too short: {} bytes (need >= 50 for AD26)",
            data.len()
        )));
    }
...
#[allow(dead_code)]
fn serialize_fill(origin: &crate::backing_store::BinaryOrigin) -> crate::Result<Vec<u8>> {
