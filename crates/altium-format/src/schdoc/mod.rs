...
impl SchDoc {
    ...
    fn parse_from_cfb(mut tracked: TrackedCfbDocument) -> Result<Self> {
        ...
        let mut records = parsed_fileheader.records;
        let mut additional_records = additional_records;
        ensure_unique_ids(&mut records);
        ensure_unique_ids(&mut additional_records);
...
/// Ensure every record that supports a unique_id has a non-empty one.
///
/// Records loaded from files may have empty unique_ids (e.g., older file versions
/// or records generated by third-party tools). This assigns a fresh UUID-based
/// identifier so the write path can index records for field preservation.
fn ensure_unique_ids(records: &mut [SchRecord]) {
    for record in records {
        if let Some(uid) = record.unique_id_mut() {
            if uid.is_empty() {
