...
use altium_format_types::constants::pin::{
    ...
    PIN_LENGTH, PIN_PACKAGE_LENGTH as PIN_PACKAGE_LENGTH_KEY,
...
use crate::sch_records::{
    ...
    SchTextFrame, parse_binary_pin, parse_component_record, parse_text_pin,
...
/// Public dump view for a single pin — used by reverse generation (dump).
#[derive(Debug, Clone)]
pub struct SchLibPinDumpView {
    pub designator: String,
    pub name: String,
    /// Lowercase electrical type string (e.g. "passive", "input").
...
/// Public dump view for a component parameter.
#[derive(Debug, Clone)]
pub struct SchLibParameterDumpView {
    pub name: String,
    pub text: String,
    pub is_hidden: bool,
}

/// Public dump view for a pin-to-pad mapping entry.
#[derive(Debug, Clone)]
pub struct SchLibPinPadMapDumpView {
    pub pin_name: String,
    pub pad_names: Vec<String>,
}

/// Public dump view for a footprint implementation.
#[derive(Debug, Clone)]
pub struct SchLibFootprintDumpView {
    pub footprint_ref: String,
    pub description: String,
    pub pin_pad_maps: Vec<SchLibPinPadMapDumpView>,
}

/// Public dump view for a graphic record.
#[derive(Debug, Clone)]
pub struct SchLibGraphicDumpView {
    /// e.g. "line", "rectangle", "arc", etc.
    pub record_type: String,
    pub unique_id: String,
    pub owner_part_id: i32,
    pub location_x_mils: f64,
    pub location_y_mils: f64,
    pub corner_x_mils: Option<f64>,
    pub corner_y_mils: Option<f64>,
    pub radius_mils: Option<f64>,
    pub secondary_radius_mils: Option<f64>,
    pub start_angle: Option<f64>,
    pub end_angle: Option<f64>,
    /// Vertex list for polyline/polygon/bezier.
    pub vertices: Vec<(f64, f64)>,
    pub text: Option<String>,
    pub file_name: Option<String>,
    pub is_solid: bool,
    /// Lowercase pen width string (e.g. "zero", "small").
    pub line_width: String,
    /// (r, g, b) stroke color.
    pub color: (u8, u8, u8),
    /// (r, g, b) fill color, if applicable.
    pub area_color: Option<(u8, u8, u8)>,
}

/// Public dump view for a full component — used by reverse generation.
#[derive(Debug, Clone)]
pub struct SchLibComponentDumpView {
    pub lib_reference: String,
    pub description: String,
    pub part_count: i32,
    pub aliases: Vec<String>,
    pub pins: Vec<SchLibPinDumpView>,
    pub parameters: Vec<SchLibParameterDumpView>,
    pub footprints: Vec<SchLibFootprintDumpView>,
    pub graphics: Vec<SchLibGraphicDumpView>,
}
...
pub(crate) struct SchLibComponentInfo {
    pub index: usize,
    pub lib_reference: String,
    pub description: String,
    pub part_count: i32,
...
pub(crate) struct SchLibPinInfo {
    pub designator: String,
    pub name: String,
    pub electrical: String,
...
pub(crate) struct SchLibRecordInfo {
    pub index: usize,
    pub record_type: i32,
    pub owner_index: i32,
    pub summary: String,
...
impl SchLib {
    ...
    pub(crate) fn ops_find_component_index_by_ref(&self, path: &str) -> Option<usize> {
    ...
    fn component_mut(&mut self, component_index: usize) -> Result<&mut SchLibComponent> {
        self.components.get_mut(component_index).ok_or_else(|| {
    ...
    pub(crate) fn ops_append_component_root(&mut self, op: &ComponentRootOp) -> Result<usize> {
        if self
            .header
            .components
            .iter()
            .any(|c| c.lib_ref == op.lib_reference)
        {
            return Err(AltiumFormatError::InvalidParamValue {
                key: LIB_REFERENCE.to_owned(),
                detail: format!(
                    "component with lib reference '{}' already exists",
                    op.lib_reference
                ),
            });
        }
        ...
        self.header.components.push(SchLibComponentIndex {
            lib_ref: op.lib_reference.clone(),
            description: String::new(),
            part_count: 1,
            aliases: Vec::new(),
        });
    ...
    pub(crate) fn ops_append_designator(
        &mut self,
        ...
        let rec = SchDesignator {
            ...
            justification: altium_format_types::TextJustification::BottomLeft,
            color: Color::BLACK,
            font_id: 1,
            is_hidden: false,
            text: text.to_owned(),
            ...
            show_name: false,
            read_only_state: altium_format_types::ParameterReadOnlyState::Name,
            unique_id: generate_unique_id(),
            ...
            text_horz_anchor: altium_format_types::TextHorzAnchor::None,
            text_vert_anchor: altium_format_types::TextVertAnchor::None,
            is_image_parameter: false,
        };
        comp.records.push(SchRecord::Designator(rec));
        self.ops_recompute_header_weight();
        Ok(())
    }

    pub(crate) fn ops_append_comment(&mut self, component_index: usize, text: &str) -> Result<()> {
        let comp = self.components.get_mut(component_index).ok_or_else(|| {
            AltiumFormatError::InvalidParamValue {
                key: "component_ref".to_owned(),
                detail: format!("component index out of range: {component_index}"),
        ...
        let rec = SchParameter {
            ...
            show_name: false,
            read_only_state: altium_format_types::ParameterReadOnlyState::None,
            unique_id: generate_unique_id(),
            description: String::new(),
            not_allow_library_synchronize: false,
            not_allow_database_synchronize: false,
        ...
        comp.records.push(SchRecord::Parameter(rec));
        self.ops_recompute_header_weight();
        Ok(())
    }

    pub(crate) fn ops_append_pin(&mut self, component_index: usize, pin: &PinOp) -> Result<()> {
        let comp = self.components.get_mut(component_index).ok_or_else(|| {
            AltiumFormatError::InvalidParamValue {
                key: "component_ref".to_owned(),
                detail: format!("component index out of range: {component_index}"),
        ...
        let mut params = ParameterCollection::new();
        params.insert(OWNER_INDEX, 0i32.to_param_value());
    ...
    pub(crate) fn ops_append_implementation_list(
        &mut self,
        component_index: usize,
    ) -> Result<usize> {
        let comp = self.components.get_mut(component_index).ok_or_else(|| {
        ...
        let stream_idx = comp.records.len() + 1;
        comp.records
    ...
    pub(crate) fn ops_append_implementation(
        &mut self,
        component_index: usize,
        implementation_list_stream_idx: usize,
        model_name: &str,
        ...
        comp.records
            .push(SchRecord::Implementation(SchImplementation {
                base: SchPrimitiveBase {
                    owner_index: implementation_list_stream_idx as i32,
                    is_not_accessible: false,
                    index_in_sheet: 0,
                    owner_part_id: 0,
                    owner_part_display_mode: 0,
    ...
    pub(crate) fn ops_append_implementation_map(
        &mut self,
        component_index: usize,
        implementation_stream_idx: usize,
    ) -> Result<usize> {
        let comp = self.components.get_mut(component_index).ok_or_else(|| {
            AltiumFormatError::InvalidParamValue {
                key: "component_ref".to_owned(),
        ...
        comp.records
            .push(SchRecord::ImplementationMap(SchImplementationMap {
                base: SchPrimitiveBase {
                    owner_index: implementation_stream_idx as i32,
                    is_not_accessible: false,
                    index_in_sheet: 0,
                    owner_part_id: 0,
                    owner_part_display_mode: 0,
    ...
    pub(crate) fn ops_append_map_definer(
        &mut self,
        component_index: usize,
        implementation_map_stream_idx: usize,
        pin_designator: &str,
        pad_designator: &str,
    ) -> Result<()> {
        let comp = self.components.get_mut(component_index).ok_or_else(|| {
        ...
        comp.records.push(SchRecord::MapDefiner(SchMapDefiner {
            base: SchPrimitiveBase {
                owner_index: implementation_map_stream_idx as i32,
                is_not_accessible: false,
                index_in_sheet: 0,
                owner_part_id: 0,
                owner_part_display_mode: 0,
    ...
    pub(crate) fn ops_append_parameter_list(
        &mut self,
        component_index: usize,
        implementation_stream_idx: usize,
    ) -> Result<()> {
        ...
        comp.records
            .push(SchRecord::ParameterList(SchParameterList {
                base: SchPrimitiveBase {
                    owner_index: implementation_stream_idx as i32,
                    is_not_accessible: false,
                    index_in_sheet: 0,
    ...
    pub(crate) fn ops_append_parameter(
        &mut self,
        component_index: usize,
        op: &ParameterOp,
    ) -> Result<()> {
        ...
        comp.records.push(SchRecord::Parameter(SchParameter {
            base: SchPrimitiveBase {
                owner_index: 0,
                is_not_accessible: false,
                index_in_sheet: 0,
    ...
    pub(crate) fn ops_append_line(&mut self, component_index: usize, op: &AddLineOp) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Line(SchLine {
    ...
    pub(crate) fn ops_append_rectangle(
        &mut self,
        component_index: usize,
        op: &AddRectangleOp,
    ) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Rectangle(SchRectangle {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_arc(&mut self, component_index: usize, op: &AddArcOp) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Arc(SchArc {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_elliptical_arc(
        &mut self,
        component_index: usize,
        ...
        let comp = self.component_mut(component_index)?;
        comp.records
            .push(SchRecord::EllipticalArc(SchEllipticalArc {
                base: primitive_base_with_owner(
                    0,
                    op.owner_part_id.unwrap_or(0),
                    op.owner_part_display_mode.unwrap_or(0),
                ...
                location: CoordPoint::new(
                    Coord::from_mils(op.cx_mils),
                    Coord::from_mils(op.cy_mils),
                ),
                radius: Coord::from_mils(op.radius_mils),
    ...
    pub(crate) fn ops_append_ellipse(
        &mut self,
        component_index: usize,
        op: &AddEllipseOp,
    ) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Ellipse(SchEllipse {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_polyline(
        &mut self,
        component_index: usize,
        op: &AddPolylineOp,
    ) -> Result<()> {
        if op.points_mils.len() < 2 {
            return Err(AltiumFormatError::InvalidParamValue {
                key: "points_mils".to_owned(),
                detail: "polyline requires at least 2 points".to_owned(),
            });
        }
        let comp = self.component_mut(component_index)?;
        let vertices = op
            ...
            .collect();
        comp.records.push(SchRecord::Polyline(SchPolyline {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
            ...
            line_width: PenWidth::try_from(op.line_width.unwrap_or(1) as u8)?,
            line_style: LineStyle::try_from(op.line_style.unwrap_or(0) as u8)?,
            start_line_shape: altium_format_types::LineShape::None,
            end_line_shape: altium_format_types::LineShape::None,
    ...
    pub(crate) fn ops_append_polygon(
        &mut self,
        component_index: usize,
        op: &AddPolygonOp,
    ) -> Result<()> {
        if op.points_mils.len() < 3 {
            return Err(AltiumFormatError::InvalidParamValue {
                key: "points_mils".to_owned(),
                detail: "polygon requires at least 3 points".to_owned(),
            });
        }
        let comp = self.component_mut(component_index)?;
        let vertices = op
            ...
            .map(|(x, y)| CoordPoint::new(Coord::from_mils(*x), Coord::from_mils(*y)))
            .collect();
        comp.records.push(SchRecord::Polygon(SchPolygon {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_bezier(
        &mut self,
        component_index: usize,
        op: &AddBezierOp,
    ) -> Result<()> {
        ...
        let vertices = op
            ...
            .map(|(x, y)| CoordPoint::new(Coord::from_mils(*x), Coord::from_mils(*y)))
            .collect();
        comp.records.push(SchRecord::Bezier(SchBezier {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_pie(&mut self, component_index: usize, op: &AddPieOp) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Pie(SchPie {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_round_rectangle(
        &mut self,
        component_index: usize,
    ...
    pub(crate) fn ops_append_label(
        ...
        comp.records.push(SchRecord::Label(SchLabel {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_text_frame(
        &mut self,
        component_index: usize,
        op: &AddTextFrameOp,
    ) -> Result<()> {
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::TextFrame(SchTextFrame {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
    ...
    pub(crate) fn ops_append_image(
        &mut self,
        component_index: usize,
        op: &AddImageOp,
    ) -> Result<()> {
        if let Some(data) = &op.image_data {
            self.embedded_images.push(SchLibEmbeddedImage {
                file_name: op.file_name.clone(),
                data: data.clone(),
            });
        }
        let comp = self.component_mut(component_index)?;
        comp.records.push(SchRecord::Image(SchImage {
            base: primitive_base_with_owner(
                0,
                op.owner_part_id.unwrap_or(0),
                op.owner_part_display_mode.unwrap_or(0),
            ...
            embed_image: op.image_data.is_some(),
            file_name: op.file_name.clone(),
            unique_id: generate_unique_id(),
        }));
        self.ops_recompute_header_weight();
    ...
    pub(crate) fn ops_add_alias(&mut self, component_index: usize, alias_name: &str) -> Result<()> {
        let canonical = self
            .header
            .components
            .get(component_index)
        ...
        if self.aliases.iter().any(|a| a.alias_name == alias_name) {
            return Err(AltiumFormatError::InvalidParamValue {
                key: "alias_name".to_owned(),
                detail: format!("alias already exists: {alias_name}"),
            });
        }
        ...
        if let Some(h) = self.header.components.get_mut(component_index) {
            h.aliases.push(alias_name.to_owned());
        }
        self.ops_recompute_header_weight();
        Ok(())
    }

    pub(crate) fn ops_remove_alias(
        &mut self,
        component_index: usize,
        alias_name: &str,
    ) -> Result<()> {
        ...
        self.aliases
            .retain(|a| !(a.alias_name == alias_name && a.canonical_name == canonical));
        if let Some(h) = self.header.components.get_mut(component_index) {
            h.aliases.retain(|a| a != alias_name);
        }
        self.ops_recompute_header_weight();
        Ok(())
    }

    pub(crate) fn ops_remove_component(&mut self, component_index: usize) -> Result<()> {
        ...
        let canonical = self.header.components[component_index].lib_ref.clone();
        self.components.remove(component_index);
        self.header.components.remove(component_index);
        self.aliases.retain(|a| a.canonical_name != canonical);
        self.ops_recompute_header_weight();
        Ok(())
    }

    pub(crate) fn ops_edit_component(
        &mut self,
        component_index: usize,
        op: &EditComponentOp,
    ) -> Result<()> {
        ...
        if let Some(v) = &op.description {
            comp.component.component_description = v.clone();
            hdr.description = v.clone();
        }
        if let Some(v) = op.part_count {
            comp.component.part_count = v;
            hdr.part_count = v;
        }
        if let Some(v) = op.display_mode_count {
            comp.component.display_mode_count = v;
        }
        if let Some(v) = op.component_kind {
            let kind = ComponentKind::try_from(v as u8)?;
            comp.component.component_kind = kind;
            comp.component.component_kind_version2 = kind;
            comp.component.component_kind_version3 = kind;
        }
    ...
    pub(crate) fn ops_edit_records(
        ...
        let targets = select_record_indices(&comp.records, selector);
        let mut changed = 0usize;
        for idx in targets {
            if apply_record_patch(&mut comp.records[idx], patch)? {
                changed += 1;
            }
        }
        Ok(changed)
    ...
    pub(crate) fn ops_remove_records(
        &mut self,
        component_index: usize,
        selector: &RecordSelector,
    ) -> Result<usize> {
        ...
        let mut remove = vec![false; comp.records.len()];
        for idx in select_record_indices(&comp.records, selector) {
            remove[idx] = true;
        }

        loop {
            let mut changed = false;
            for (idx, rec) in comp.records.iter().enumerate() {
                if remove[idx] {
                    continue;
                }
                let owner_index = record_owner_index(rec);
                if owner_index > 0 {
                    let parent = (owner_index - 1) as usize;
        ...
        let mut old_to_new = vec![None; comp.records.len()];
        ...
        for (old_idx, rec) in comp.records.drain(..).enumerate() {
            if remove[old_idx] {
                continue;
            }
            kept.push(rec);
        ...
        for rec in &mut kept {
            let owner = record_owner_index(rec);
            if owner > 0 {
                let old_parent = (owner - 1) as usize;
                if let Some(new_parent) = old_to_new[old_parent] {
                    set_record_owner_index(rec, new_parent as i32 + 1);
                }
            }
        }
        comp.records = kept;
        comp.component.all_pin_count = comp
            .records
            .iter()
            .filter(|r| matches!(r, SchRecord::Pin(_)))
            .count() as i32;
    ...
    pub(crate) fn ops_query_components(&self, pattern: Option<&str>) -> Vec<SchLibComponentInfo> {
        let mut out = Vec::new();
        for idx in 0..self.components.len() {
            let Some(h) = self.header.components.get(idx) else {
                continue;
            };
            if let Some(p) = pattern {
                if !h.lib_ref.contains(p) && !h.description.contains(p) {
                    continue;
                }
            }
            ...
            let pin_count = comp
                ...
                .filter(|r| matches!(r, SchRecord::Pin(_)))
                .count() as i32;
            let has_footprint = comp.records.iter().any(|r| {
                matches!(
                    r,
                    SchRecord::ImplementationList(_) | SchRecord::Implementation(_)
                )
            });
            out.push(SchLibComponentInfo {
                index: idx,
                lib_reference: h.lib_ref.clone(),
                description: h.description.clone(),
                part_count: h.part_count,
                pin_count,
    ...
    pub(crate) fn ops_query_pins(&self, component_index: usize) -> Result<Vec<SchLibPinInfo>> {
        let comp = self.components.get(component_index).ok_or_else(|| {
            AltiumFormatError::InvalidParamValue {
                key: "component_ref".to_owned(),
                detail: format!("component index out of range: {component_index}"),
            }
        ...
        for rec in &comp.records {
            if let SchRecord::Pin(pin) = rec {
                out.push(SchLibPinInfo {
                    designator: pin.designator.clone(),
                    name: pin.name.clone(),
                    electrical: format!("{:?}", pin.electrical),
                    location: pin.location,
                    length: pin.pin_length,
                    orientation: pin.orientation.to_degrees(),
    ...
    pub(crate) fn ops_query_records(
        &self,
        component_index: usize,
        record_type: Option<i32>,
    ) -> Result<Vec<SchLibRecordInfo>> {
        let comp = self.components.get(component_index).ok_or_else(|| {
            AltiumFormatError::InvalidParamValue {
                key: "component_ref".to_owned(),
                detail: format!("component index out of range: {component_index}"),
        ...
        for (idx, rec) in comp.records.iter().enumerate() {
            ...
            if let Some(want) = record_type {
                if want != ty {
                    continue;
                }
            }
            out.push(SchLibRecordInfo {
                index: idx,
                record_type: ty,
                owner_index: record_owner_index(rec),
    ...
    fn ops_recompute_header_weight(&mut self) {
        ...
        for (idx, comp) in self.components.iter().enumerate() {
            let aliases = self
                ...
                .map(|v| v.aliases.len())
                .unwrap_or(0);
            weight += comp.records.len() + aliases;
        }
        self.header.weight = weight as i32;
    }
    ...
    pub fn dump_components(&self) -> Vec<SchLibComponentDumpView> {
        self.components.iter().zip(self.header.components.iter()).map(|(comp, hdr)| {
            let pins = comp.records.iter().filter_map(|r| {
                if let SchRecord::Pin(p) = r {
                    Some(SchLibPinDumpView {
                        designator: p.designator.clone(),
                        name: p.name.clone(),
                        electrical: format!("{:?}", p.electrical).to_lowercase(),
                        location_x_mils: p.location.x.to_mils(),
                        location_y_mils: p.location.y.to_mils(),
                        length_mils: p.pin_length.to_mils(),
                        orientation: p.orientation.to_degrees(),
            ...
            let parameters = comp.records.iter().filter_map(|r| {
                if let SchRecord::Parameter(p) = r {
                    // Skip special parameters (Designator, Comment)
                    if p.name == "Designator" || p.name == "Comment" {
                        return None;
                    }
                    Some(SchLibParameterDumpView {
                        name: p.name.clone(),
                        text: p.text.clone(),
                        is_hidden: p.is_hidden,
                    })
            ...
            // Collect footprint implementations
            let footprints = comp.records.iter().filter_map(|r| {
                if let SchRecord::Implementation(imp) = r {
                    if imp.model_type.eq_ignore_ascii_case("PCBLIB") {
                        let pin_pad_maps = comp.records.iter().filter_map(|r2| {
                            if let SchRecord::MapDefiner(md) = r2 {
                                if md.base.owner_index == imp.base.owner_index {
                                    Some(SchLibPinPadMapDumpView {
                                        pin_name: md.des_intf.clone(),
                                        pad_names: md.des_imps.clone(),
                                    })
                                } else {
                                    None
                                }
                        ...
                        Some(SchLibFootprintDumpView {
                            footprint_ref: imp.model_name.clone(),
            ...
            // Collect graphics (excluding pins, parameters, implementation-related records)
            let graphics = comp.records.iter().filter_map(|r| {
                match r {
                    SchRecord::Line(g) => Some(SchLibGraphicDumpView {
                        record_type: "line".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        corner_x_mils: Some(g.corner.x.to_mils()),
                        corner_y_mils: Some(g.corner.y.to_mils()),
                        radius_mils: None,
                        secondary_radius_mils: None,
                        start_angle: None,
                        end_angle: None,
                        vertices: vec![],
                        text: None,
                        file_name: None,
                        is_solid: false,
                        line_width: format!("{:?}", g.line_width).to_lowercase(),
                        color: (g.color.r(), g.color.g(), g.color.b()),
                        area_color: None,
                    }),
                    SchRecord::Rectangle(g) => Some(SchLibGraphicDumpView {
                        record_type: "rectangle".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        ...
                        line_width: format!("{:?}", g.line_width).to_lowercase(),
                        color: (g.color.r(), g.color.g(), g.color.b()),
                        area_color: Some((g.area_color.r(), g.area_color.g(), g.area_color.b())),
                    }),
                    SchRecord::Arc(g) => Some(SchLibGraphicDumpView {
                        record_type: "arc".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        corner_x_mils: None,
                        corner_y_mils: None,
                        radius_mils: Some(g.radius.to_mils()),
                        secondary_radius_mils: None,
                        start_angle: Some(g.start_angle.0),
                        end_angle: g.end_angle.map(|a| a.0),
                        vertices: vec![],
                        text: None,
                        file_name: None,
                        is_solid: false,
                    ...
                    SchRecord::EllipticalArc(g) => Some(SchLibGraphicDumpView {
                        record_type: "elliptical_arc".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        ...
                        corner_y_mils: None,
                        radius_mils: Some(g.radius.to_mils()),
                        secondary_radius_mils: Some(g.secondary_radius.to_mils()),
                        start_angle: Some(g.start_angle.0),
                        end_angle: g.end_angle.map(|a| a.0),
                        vertices: vec![],
                        text: None,
                    ...
                    SchRecord::Ellipse(g) => Some(SchLibGraphicDumpView {
                        record_type: "ellipse".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                    ...
                    SchRecord::Pie(g) => Some(SchLibGraphicDumpView {
                        record_type: "pie".to_owned(),
                    ...
                    SchRecord::Polyline(g) => Some(SchLibGraphicDumpView {
                        record_type: "polyline".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: 0.0,
                        location_y_mils: 0.0,
                        ...
                        start_angle: None,
                        end_angle: None,
                        vertices: g.vertices.iter().map(|v| (v.x.to_mils(), v.y.to_mils())).collect(),
                        text: None,
                        file_name: None,
                        is_solid: false,
                        line_width: format!("{:?}", g.line_width).to_lowercase(),
                    ...
                    SchRecord::Polygon(g) => Some(SchLibGraphicDumpView {
                        record_type: "polygon".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: 0.0,
                        location_y_mils: 0.0,
                    ...
                    SchRecord::Bezier(g) => Some(SchLibGraphicDumpView {
                        record_type: "bezier".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: 0.0,
                        location_y_mils: 0.0,
                    ...
                    SchRecord::Label(g) => Some(SchLibGraphicDumpView {
                        record_type: "label".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        ...
                        end_angle: None,
                        vertices: vec![],
                        text: Some(g.text.clone()),
                        file_name: None,
                        is_solid: false,
                        line_width: "zero".to_owned(),
                        color: (g.color.r(), g.color.g(), g.color.b()),
                        area_color: None,
                    }),
                    SchRecord::TextFrame(g) => Some(SchLibGraphicDumpView {
                        record_type: "text_frame".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                    ...
                    SchRecord::Image(g) => Some(SchLibGraphicDumpView {
                        record_type: "image".to_owned(),
                        unique_id: g.unique_id.clone(),
                        owner_part_id: g.base.owner_part_id,
                        location_x_mils: g.location.x.to_mils(),
                        location_y_mils: g.location.y.to_mils(),
                        ...
                        vertices: vec![],
                        text: None,
                        file_name: Some(g.file_name.clone()),
                        is_solid: false,
                        line_width: "zero".to_owned(),
                        color: (0, 0, 0),
                        area_color: None,
            ...
            SchLibComponentDumpView {
                lib_reference: hdr.lib_ref.clone(),
                description: hdr.description.clone(),
                part_count: hdr.part_count,
                aliases: hdr.aliases.clone(),
                pins,
                parameters,
                footprints,
                graphics,
...
fn select_record_indices(records: &[SchRecord], selector: &RecordSelector) -> Vec<usize> {
    ...
    for (idx, rec) in records.iter().enumerate() {
        let matched = match selector {
            ...
            RecordSelector::ByIndex(v) => idx == *v,
...
fn set_record_owner_index(rec: &mut SchRecord, owner_index: i32) {
    match rec {
