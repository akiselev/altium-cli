...
mod tests {
    ...
    fn assert_all_consumed_succeeds_when_all_streams_are_read() {
        ...
        let (root_storages, root_streams) = doc
            .list_entries("/")
            .expect("list_entries root must succeed");
        // Read each top-level stream using its full path.
        for name in &root_streams {
            let full = format!("/{name}");
            doc.read_stream(&full)
                .expect("top-level stream must be readable");
        }
        ...
        for storage_name in &root_storages {
            ...
            let (_, nested_streams) = doc
                .list_entries(&storage_path)
                .expect("list_entries storage must succeed");
            for stream_name in nested_streams {
                let full = format!("{storage_path}/{stream_name}");
                doc.read_stream(&full)
                    .expect("nested stream must be readable");
            }
        }
        doc.assert_all_consumed()
            .expect("all streams consumed; assert must return Ok");
    }
    ...
    fn assert_all_consumed_fails_when_nothing_is_read() {
        ...
        match result {
            Err(AltiumFormatError::UnconsumedStreams { paths }) => {
                ...
                assert!(
                    paths.contains(&"/FileHeader".to_owned()),
                    "/FileHeader must be unconsumed"
                );
                assert!(
                    paths.contains(&"/Storage".to_owned()),
                    "/Storage must be unconsumed"
                );
                assert!(
