...
use crate::v2::store::{DocRef, DocumentMeta, DocumentStore};
...
impl PcbDoc {
    ...
    pub fn open<R: Read + Seek>(mut reader: R) -> Result<Self> {
        ...
        crate::v2::semantic_ids::compute_all_ids(&mut store, "dtid:pcbdoc", &doc_key);
...
mod tests {
    ...
    fn open_save_roundtrip_single_track_section() {
        ...
        let payload = match track_origin {
            RecordOrigin::Binary(crate::v2::backing_store::BinaryOrigin { raw_block, .. }) => {
                raw_block
            }
    ...
    fn add_primitive_record_and_query_roundtrip() {
        ...
        let track = crate::v2::records::PcbTrackRecord::from_origin(templates::pcb_track_default());
    ...
    fn add_primitive_record_rejects_mismatched_section_type() {
        ...
        let arc = crate::v2::records::PcbArcRecord::from_origin(templates::pcb_arc_default());
        let err = doc
