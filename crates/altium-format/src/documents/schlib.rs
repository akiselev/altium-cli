...
impl SchLib {
    ...
    pub fn storage_meta(&self) -> crate::documents::schdoc_streams::SchDocStorageStreamMeta {
        let store = self.store.borrow();
        match store.meta() {
            DocumentMeta::SchLib { storage_meta, .. } => storage_meta.clone(),
            _ => crate::documents::schdoc_streams::SchDocStorageStreamMeta::default(),
        }
    }
    ...
    pub fn open<R: Read + Seek>(mut reader: R) -> Result<Self> {
        ...
        let file_hash = crate::semantic_ids::blake3_content_hash(&raw_bytes);
...
impl crate::traits::DocumentQuery<crate::handles::SchComponent> for SchLib {
    ...
    fn query_all(
        ...
        for &group_id in store.group_ids() {
            ...
            let all = std::slice::from_ref(node);
            if !evaluate(&parsed, all).is_empty() {
                handles.push(crate::handles::SchComponentHandle::new(
                    self.store.clone(),
                    group_id,
                ));
            }
...
mod tests {
    ...
    fn schlib_query_component() {
        use crate::handles::SchComponent;
    ...
    fn schlib_query_modifies_via_handle() {
        use crate::handles::SchComponent;
        use crate::newtypes::LibReference;
        use crate::traits::DocumentQuery;
