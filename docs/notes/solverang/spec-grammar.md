...
Extends the existing spec language (`.schlib-spec`, `.pcblib-spec`) with a new
`.pcbdoc-spec` domain for board-level placement constraints and design rule
specifications.
...
altium placement plan board-layout.pcbdoc-spec
altium placement plan board-layout.pcbdoc-spec --target my-board.PcbDoc

# Apply: solve + write component positions
altium placement apply board-layout.pcbdoc-spec

# DRC: check design rules against existing board
altium drc my-board.PcbDoc                           # use rules from board
altium drc my-board.PcbDoc --rules rules.pcbdoc-spec # use rules from spec

# DRC with JSON output
altium drc my-board.PcbDoc --json

# Dump: extract current placement as spec
altium placement dump my-board.PcbDoc                # → my-board.pcbdoc-spec
```
...
- `.schlib-spec` → SchLib
- `.pcblib-spec` → PcbLib
- `.prjpcb-spec` → PrjPcb (already exists)
