...
Pipeline:
  ...
  2. For each pin swap:
     - Find the .sch file containing the component
     - Update net label assignments on the swapped pins
  3. For each part swap:
     - Find both components in the .sch
     - Exchange their net connections
  4. Write updated .sch files
```
...
```
// In board.sch (or a generated swap overlay file):
...
```
// board-swaps.sch (auto-generated by autoplacer)
...
The main `.pcb` imports this:

```
placement {
    import "board-swaps.sch"   // pin/part swap assignments
...
SWAP OPTIMIZATION
  ...
  Swap overlay written to: board-swaps.sch
```
...
Phase 5: DRC verification (existing)
    ↓
Output: Updated .pcb + board-swaps.sch
```
...
│  INPUT FILES                                                     │
│                                                                   │
│  board.pcb ── placement constraints, autoplace directives │
...
│  OUTPUT FILES                                                     │
│                                                                   │
│  board.pcb ── updated with explicit positions            │
│  board-swaps.sch ── pin/part/gate swap overlay (NEW)    │
...
- Parse/rewrite .sch with swap overlay
- CLI `--swap-overlay` flag
- Import mechanism in `.pcb` spec
- **Effort**: 1 week
